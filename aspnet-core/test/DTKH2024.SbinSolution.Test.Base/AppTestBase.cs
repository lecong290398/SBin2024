...
using DTKH2024.SbinSolution.Test.Base.TestData;
...
namespace DTKH2024.SbinSolution.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="SbinSolutionDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(SbinSolutionDbContext context)
            {
        ...
        protected void UsingDbContext(Action<SbinSolutionDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SbinSolutionDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<SbinSolutionDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<SbinSolutionDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SbinSolutionDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SbinSolutionDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SbinSolutionDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SbinSolutionDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<SbinSolutionDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<SbinSolutionDbContext, Task<TResult>> func)
        {
            TResult result;
