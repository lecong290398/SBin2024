...
void loop()
{
    ...
    bool isCoRac = CoRac();
    bool isRacKimLoaiDay = RacKimLoaiDay();
    bool isRacNhuaDay = RacNhuaDay();
    bool isRacKhongXacDinhDay = RacKhongXacDinhDay();
    // Không có rác
    if (!isCoRac)
    {
    ...
    else if (isRacNhuaDay)
    {
        GuiCmdDayRacNhua();
        RcServoVeGocMin();
    }
    else if (isRacKhongXacDinhDay)
    {
    ...
    if (isCoRac)
    {
        ...
        else if (RacNhua() && !isRacNhuaDay)
        {
            Serial.println("Rac nhựa");
        ...
        else if(!isRacKhongXacDinhDay)
        {
            ...
            // đến khi nào cảm biến ( pin A4) xác nhận rác đã rơi ra ngoài
            // thì sau đó thì chuyển về góc 0 độ
            Serial.println("Rac  không xác định");
            do
            {
                RcServoTangGoc();
            } while (CoRac());
