...
#pragma endregion
void processPage(uint8_t pageNum) {
...
void TaskWiFi(void *pvParameters) {
    while (true) {
        connectWiFi();
        vTaskDelay(60000 / portTICK_PERIOD_MS); // Reconnect every 60 seconds
    }
}

void TaskTokenHandler(void *pvParameters) {
    while (true) {
        HandlerToken();
        vTaskDelay(60000 / portTICK_PERIOD_MS); // Handle token every 60 seconds
    }
}

void TaskPageProcessor(void *pvParameters) {
    while (true) {
        uint8_t pageNum = dwc.getPage();
        processPage(pageNum);
        vTaskDelay(200 / portTICK_PERIOD_MS); // Process page every 200 ms
    }
}
...
void setup() {
    ...
    xTaskCreate(TaskWiFi, "TaskWiFi", 2048, NULL, 1, NULL);
    xTaskCreate(TaskTokenHandler, "TaskTokenHandler", 2048, NULL, 1, NULL);
    xTaskCreate(TaskPageProcessor, "TaskPageProcessor", 2048, NULL, 1, NULL);
}
