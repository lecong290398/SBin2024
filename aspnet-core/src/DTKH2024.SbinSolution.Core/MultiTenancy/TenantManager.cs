...
using DTKH2024.SbinSolution.MultiTenancy.Demo;
        ...
        IEventHandler<RecurringPaymentSucceedEventData>
    {
        ...
        )
        {
            ...
            if (isInTrialPeriod && !subscriptionEndDate.HasValue)
            {
                throw new UserFriendlyException(LocalizationManager.GetString(
                    SbinSolutionConsts.LocalizationSourceName, "TrialWithoutEndDateErrorMessage"));
            }
        ...
        public async Task CheckEditionAsync(int? editionId, bool isInTrialPeriod)
        {
            await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
            {
                ...
                var error = LocalizationManager.GetSource(SbinSolutionConsts.LocalizationSourceName)
        ...
        public void HandleEvent(RecurringPaymentSucceedEventData eventData)
        {
            _unitOfWorkManager.WithUnitOfWork(() =>
            {
                ...
                if (editionItem == null || !editionItem.Product.Equals(SbinSolutionConsts.ProductName))
                {
                    Logger.Warn(
                        $"This is not a payment for {SbinSolutionConsts.ProductName} for edition subscription cycle! " +
